...
import 'package:newtest/module/notification/controller/notif_data_controller.dart';
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  List<DataRow> tableContents() {
    ...
    for (int i = 0; i < NotifDataController.notifDataList.length; i++) {
      createdAt =
          NotifDataController.notifDataList[i].createdAt.substring(0, 19);
      notifSchedule =
          NotifDataController.notifDataList[i].notifSchedule.substring(0, 19);
  ...
  Future<void> onCreateButtonPressed() async {
    ...
    final createdAt = notifKey;
    final notifSchedule = DateTime.now().add(Duration(minutes: _minute));

    NotifDataController.notifDataList.add(NotifData(
        notifId: notifId,
        createdAt: createdAt,
        notifSchedule: notifSchedule.toString(),
